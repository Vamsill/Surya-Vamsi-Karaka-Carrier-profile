...
const ProjectCard: React.FC<Project> = ({ title, description, techStack, links, details }) => (
    <div className="bg-bg-secondary border border-slate-800/50 p-6 rounded-lg shadow-lg hover:shadow-accent/10 transition-all duration-300 transform hover:-translate-y-2 flex flex-col h-full">
        <div className="flex-grow">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
                {techStack.map(tech => (
                    <span key={tech} className="bg-accent-soft text-accent text-xs font-mono px-2 py-1 rounded">
                        {tech}
                    </span>
                ))}
            </div>
